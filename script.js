    ...
    function onSuccess(position){
        ...
        fetch(`https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=99373edc93964740872efcf65e81882a`)
       //parsing json data into javascript object and returning it and in another then function receiving the object that is sent by the api
        .then(response => response.json()).then(response =>{
            ...
            let {county, postcode, country, road, suburb, state} = allDetails; //getting country, postcode, country properties value from allDetails obj
            //button.innerText = `${county} ${postcode}, ${country}, ${road}`; //passing these value to the button innerText
            button.innerText = `${road}, ${suburb}, ${county} ${postcode}, ${state}, ${country}`; //passing these value to the button innerText
